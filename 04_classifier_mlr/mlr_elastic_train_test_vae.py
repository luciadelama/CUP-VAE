...
regularization using VAE latent embeddings as input features.

The script:
1. Loads latent embeddings that already contain sample labels and split names.
2. Encodes tissue labels as integers.
3. Standardizes features using only the training set.
...
# Input features are latent embeddings generated by a VAE
# =========================
...
EMBEDDINGS_PATH = "../results/vae_outputs_841778/latent_embeddings_last.csv"
...
OUTPUT_DIR = Path(f"mlr_vae_outputs_{JOB_ID}")
...
# =========================
# Load latent embeddings and related metadata
# =========================

# The latent embedding file is expected to include sample_id, tissue, and split columns.
latent_df = pd.read_csv(EMBEDDINGS_PATH)

# These files are loaded for traceability, but the current workflow uses the
# tissue labels and split assignments already stored in the embedding file.
...
# =========================
# Preprocessing
# Fit StandardScaler on the training set only, then apply it to validation and test data
# =========================
...
checkpoint = {
    ...
    "config": {
        ...
        "num_epochs": NUM_EPOCHS,
        "gradient_clipping_max_norm": GRAD_CLIP_MAX_NORM,
        "input_transform": "StandardScaler on latent embeddings",
